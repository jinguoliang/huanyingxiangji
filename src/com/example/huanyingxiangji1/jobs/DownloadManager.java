package com.example.huanyingxiangji1.jobs;
...
public class DownloadManager {
    ...
    private List<Jobs> downloadInfoList;
    ...
    /*package*/ DownloadManager(Context appContext) {
        ...
        try {
            downloadInfoList = db.findAll(Selector.from(Jobs.class));
        } catch (DbException e) {
            LogUtils.e(e.getMessage(), e);
        }
        if (downloadInfoList == null) {
            downloadInfoList = new ArrayList<Jobs>();
        }
    }
    ...
    public Jobs getDownloadInfo(int index) {
    ...
    public void addNewDownload(String url, String fileName, String target,
        ...
        final Jobs downloadInfo = new Jobs();
    ...
    public void resumeDownload(int index, final RequestCallBack<File> callback) throws DbException {
        final Jobs downloadInfo = downloadInfoList.get(index);
        resumeDownload(downloadInfo, callback);
    }

    public void resumeDownload(Jobs downloadInfo, final RequestCallBack<File> callback) throws DbException {
        HttpUtils http = new HttpUtils();
        http.configRequestThreadPoolSize(maxDownloadThread);
    ...
    public void removeDownload(int index) throws DbException {
        Jobs downloadInfo = downloadInfoList.get(index);
        removeDownload(downloadInfo);
    }

    public void removeDownload(Jobs downloadInfo) throws DbException {
    ...
    public void stopDownload(Jobs downloadInfo) throws DbException {
        HttpHandler<File> handler = downloadInfo.getHandler();
        if (handler != null && !handler.isCancelled()) {
            handler.cancel();
        } else {
    ...
    public void stopAllDownload() throws DbException {
        for (Jobs downloadInfo : downloadInfoList) {
            HttpHandler<File> handler = downloadInfo.getHandler();
            if (handler != null && !handler.isCancelled()) {
                handler.cancel();
            } else {
    ...
    public class ManagerCallBack extends RequestCallBack<File> {
        ...
        private ManagerCallBack(Jobs downloadInfo, RequestCallBack<File> baseCallBack) {
            this.baseCallBack = baseCallBack;
